...
class Stokes(SphericalIntegral, MeanStokesKernel):
    ...
    def innermost_interpolation(self, latP, lonP, rgrid, nzn, dlat, dlon):
        # innermost zone
        lat_iz_bnds = [latP + dlat * nzn, latP - dlat * nzn]
        lon_iz_bnds = [lonP - dlon * nzn, lonP + dlon * nzn]
        iz_grid = rgrid.sel(lat=slice(*lat_iz_bnds), lon=slice(*lon_iz_bnds))
    ...
    def innermost_zone(self, gravity_anomaly, lat, dlatr, dlonr):
        s0 = self.ref_radius * np.sqrt((dlonr * dlatr * np.cos(np.radians(lat))) / np.pi)
    ...
    def evaluate_point(self, lat, lon, degrees=True):
        ...
        dlon = np.abs(np.diff(data_grid.lon.values)[0])
        dlat = np.abs(np.diff(data_grid.lat.values)[0])
    ...
    def evaluate_grid(self, bounds):
        ...
        # TODO: check if bounds is in range

        calc_grid = self.data.sel(
            lat=slice(*lat_calc_bnds),
            lon=slice(*lon_calc_bnds))

        lat_bnds = [lat_calc_bnds[0] + self.psi2, lat_calc_bnds[1] - self.psi2]
        lon_bnds = [lon_calc_bnds[0] - self.psi2, lon_calc_bnds[1] + self.psi2]

        # TODO: check if bounds is in range
        data_grid = self.data.sel(lat=slice(*lat_bnds), lon=slice(*lon_bnds))

        #lats = calc_grid.lat.values
        dlon = np.abs(np.diff(calc_grid.lon.values)[0])
        dlat = np.abs(np.diff(calc_grid.lat.values)[0])
        dlonr, dlatr = np.radians([dlon, dlat])
