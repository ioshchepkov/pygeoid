...
class Prism(_PotentialBase):
    ...
    Parameters
    ----------
    bounds : (x1, x2, y1, y2, z1, z2), ~pygeoid.conventions.units.Quantity
    ...
    Notes
    -----
    The formulas from Nagy et al. are used in this class.
    ...
    @u.quantity_input
    def __init__(
        self, bounds: PrismBounds, density: u.kg / u.m**3 = 1.0 * u.kg / u.m**3
    ):
    ...
    def _potential(self, position):
        ...
        Parameters
        ----------
        position : ~pygeoid.geometry.LocalFrame
    ...
    def _hessian(self, position):
        ...
        gxz = self.gxz(position).value
        gyz = self.gyz(position).value

        tensor = np.asarray([[gxx, gxy, gxz], [gxy, gyy, gyz], [gxz, gyz, gzz]])
    ...
    def invariants(self, position):
        ...
        i_2 = 0.5 * (np.trace(tensor, axis1=-1) ** 2 - np.trace(tensor**2, axis1=-1))
