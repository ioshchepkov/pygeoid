...
from pygeoid.conventions.constants.iers2010 import k20
from pygeoid.earth.gravity import spherical_harmonics as _expand
from pygeoid.earth.gravity.centrifugal import Centrifugal as _Centrifugal
from pygeoid.earth.gravity.normal import LevelEllipsoid as _LevelEllipsoid
from pygeoid.fields.potential.base import PotentialBase as _PotentialBase
...
class GlobalGravityFieldModel:
    ...
    service and described in the Scientific Technical Report STR09/02.

    Parameters
    ----------
    coeffs : ~pygeoid.conventions.units.Quantity
    ...
    gm : ~pygeoid.conventions.units.Quantity
        Gravitational parameter that is associated with the gravitational
        potential coefficients.
    r0 : ~pygeoid.conventions.units.Quantity
    ...
    def to_tide_system(self, tide_system: str):
        ...
        system (:math:`C_{20}^{ZT}`). Their relationship is given by Sanchez
        et al.:

        .. math::
            C_{20}^{ZT} = C_{20}^{NT} +
            k_{20} \frac{r_0}{GM} A' \left(\frac{r_0}{a}\right)^2
        ...
        :math:`a` are given by Sanchez et al.

        References
        ----------
        - Sanchez, L. et al. Strategy for the realisation of the
        ...
        if tide_system not in ("zero-tide", "non-tidal"):
            ...
            raise ValueError(
                "Nothing to do, the model is already in the desired tide system!"
            )
    ...
    def gravity(self, position) -> u.m / u.s**2:
        ...
        ~pygeoid.conventions.units.Quantity
            Gravity.
        """
        return self._gravity.gradient(position)
    ...
    @u.quantity_input
    def height_anomaly_ell(self, position, ref_pot: u.m**2 / u.s**2 = None) -> u.m:
    ...
    def vertical_deflection_abs(self, position) -> u.arcsec:
        ...
        spherical approximation (see Jekeli, eq. 26-28).

        References
        ----------
        - Jekeli, C. An analysis of vertical deflections derived
          from high-degree spherical harmonic models.
          Journal of Geodesy 73, 10-22 (1999).

        """

        dapot_lat = self._anomalous._derivative(position, "lat")
        dapot_lon = self._anomalous._derivative(position, "lon")
        dapot_rad = self._anomalous._derivative(position, "r")
...
class SHGravPotential(_PotentialBase):
    ...
    def r_derivative(self, position) -> u.m / u.s**2:
        ...
        ~pygeoid.conventions.units.Quantity
            Radial derivative.
        """
        out = self._derivative(position, "radius")
        if self._coeffs.omega is not None:
            out += self.centrifugal.differential(
    ...
    def lon_derivative(self, position):
        ...
        if self._coeffs.omega is not None:
            centr_d_lon = self.centrifugal.differential(
                position, SphericalDifferential
            ).d_lon
            scale = (
                u.rad
    ...
    def _differential(self, position, *args, **kwargs):
        ...
        if self._coeffs.omega is not None:
            out += self.centrifugal.differential(position, SphericalDifferential)
