...
from pygeoid.earth.gravity.integrals.stokes import StokesKernel
from pygeoid.numerics.legendre import legendre as _legendre
...
def _qn_numerical(t, n_max, **kwargs):
    ...
    def f(x, n):
        return StokesKernel()._kernel_t(x) * _legendre(n, x)[-1]
...
def molodensky_truncation_coefficients(
    ...
    **kwargs
    ...
    The function calculates this integral by Hagiwara's method or by
    the numerical integration with `scipy.intagrate.quad`.

    References
    ----------
    - Molodensky MS, Yeremeyev VF, Yurkina MI (1962) Methods for study
      ...
      Jerusalem.
    - Hagiwara Y (1976) A new formula for evaluating the truncation
      error coefficient. Bulletin Geodesique 50:131-135.
...
def _rnk_tril_num(r, t):
    for i, j in zip(*np.tril_indices_from(r)):
        if i != j:
            ...
            def f(x, ni, ki):
                return _legendre(ni, x)[-1] * _legendre(ki, x)[-1]

            args = (i, j)
        else:

            def f(x, ni):
                return _legendre(ni, x)[-1] ** 2
...
def paul_coefficients(
    ...
    **kwargs
    ...
    Note that in the original article the Paul's coefficients are denoted as
    :math:`R_{n,k}`.

    References
    ----------
    - Paul MK (1973) A method of evaluating the truncation error
      coefficients for geoidal height. Bull Geodesique 110:413-425.
