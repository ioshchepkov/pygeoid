...
class MeanKernel:
    ...
    def asymptotic_kernel(self, spherical_distance, degrees=True, **kwargs):
        ...
        return self._asymptotic_kernel(spherical_distance, degrees=degrees, **kwargs)
    ...
    def mean_kernel(self, latp, lonp, latq, lonq, dlat, dlon):
        ...
        # cell boundaries
        x1, y1 = latlon_to_metres(latq - 0.5 * dlat, lonq - 0.5 * dlon, origin)
        x2, y2 = latlon_to_metres(latq + 0.5 * dlat, lonq + 0.5 * dlon, origin)
...
class MeanStokesKernelBase(MeanKernel):
    def _asymptotic_kernel(self, spherical_distance, degrees=True):
        psi = self._check_spherical_distance(
            spherical_distance=spherical_distance, degrees=degrees
        )
