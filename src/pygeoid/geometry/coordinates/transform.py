...
def _cartesian_to_geodetic(x, y, z, ell, degrees=True):
    ...
    Notes
    -----
    The algorithm of H. Vermeille is used for this transformation.

    References
    ----------
    - Vermeille, H., 2011. An analytical method to transform geocentric
...
def geodetic_to_spherical(lat: u.deg, lon: u.deg, height: u.m, ell):
    ...
    Returns
    -------
    lat, lon : ~pygeoid.conventions.units.Quantity
...
@u.quantity_input
def latlon_to_planar(lat: u.deg, lon: u.deg, origin: tuple[u.deg, u.deg]):
    r"""Convert geographical coordinates to planar approximation.
    ...
    between the origin point and the points specified by `(lat, lon)`.
    ...
    Returns
    -------
    east, north : ~pygeoid.conventions.units.Quantity
    ...
    References
    ----------
    - Hirt C, Featherstone WE, Claessens SJ (2011) On the accurate numerical
...
def polar_to_cartesian(theta: u.deg, radius: u.m):
    ...
    Parameters
    ----------
    theta : ~pygeoid.conventions.units.Quantity
        Polar angle.
    ...
    Returns
    -------
    x, y : ~pygeoid.conventions.units.Quantity
        Local east-north-uo cartesian coordinates.
    """
