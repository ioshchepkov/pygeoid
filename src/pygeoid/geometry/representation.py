...
class GeodeticRepresentation(BaseRepresentation):
    ...
    def unit_vectors(self):
        ...
        return {
            ...
            "height": CartesianRepresentation(
    ...
    def scale_factors(self):
        ...
        sf_height = np.broadcast_to(1.0 * u.one, self.shape, subok=True)
        return {"lon": sf_lon, "lat": sf_lat, "height": sf_height}

    def to_cartesian(self):
        x, y, z = u.Quantity(
            transform.geodetic_to_cartesian(
                self.lat, self.lon, self.height, self._ellipsoid
            )
        )
...
class EllipsoidalHarmonicRepresentation(BaseRepresentation):
    ...
    def __init__(self, rlat, lon=None, u_ax=None, ell=None, copy=True):
        ...
        if not self.u_ax.unit.is_equivalent(u.m):
            raise u.UnitTypeError(
                f"{self.__class__.__name__} requires u_ax with units of length."
            )
    ...
    def unit_vectors(self):
        ...
        uv_u_ax = (uwk * cosrlat * coslon, uwk * cosrlat * sinlon, sinrlat / w)
    ...
    def to_cartesian(self):
        x, y, z = u.Quantity(
            transform.ellipsoidal_to_cartesian(
                self.rlat, self.lon, self.u_ax, self._ellipsoid
            )
        )
        return CartesianRepresentation(x, y, z, copy=False)
