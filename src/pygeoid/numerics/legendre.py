...
__all__ = ["legendre", "lplm", "lplm_d1"]


def legendre(lmax, x, *args, **kwargs):
    """Legendre polynomials through degree ``lmax``.

    This is a thin wrapper around :func:`pyshtools.legendre.PLegendre`.
    """
    from pyshtools.legendre import PLegendre as _PLegendre

    return _PLegendre(lmax, x, *args, **kwargs)
...
def lplm(lmax, x):
    ...
    Plm(x) : (lmax + 1, lmax + 1) array
        Values of Plm(x) for all orders and degrees.
    """
    from pyshtools.legendre import PlmBar
