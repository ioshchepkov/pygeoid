...
def test_against_GRS80():
    ...
    np.testing.assert_almost_equal(
        ell.linear_eccentricity.value, 521854.0097, decimal=4
    )
    np.testing.assert_almost_equal(
        ell.polar_curvature_radius.value, 6399593.6259, decimal=4
    )
    np.testing.assert_almost_equal(
        ell.eccentricity_squared.value, 0.00669438002290, decimal=14
    )
    np.testing.assert_almost_equal(
        ell.second_eccentricity_squared.value, 0.00673949677548, decimal=14
    )
    np.testing.assert_almost_equal(ell.flattening.value, 0.00335281068118, decimal=14)
    np.testing.assert_almost_equal(
        ell.reciprocal_flattening.value, 298.257222101, decimal=9
    )
    np.testing.assert_almost_equal(
        ell.quadrant_distance.value, 10001965.7293, decimal=4
    )
    np.testing.assert_almost_equal(
        ell.surface_area.value * 1e-014, 5.10065622, decimal=8
    )
    np.testing.assert_almost_equal(ell.volume.value * 1e-021, 1.08320732, decimal=8)

    # mean radius
    kind = "arithmetic"
    np.testing.assert_almost_equal(ell.mean_radius(kind).value, 6371008.7714, decimal=4)
    kind = "same_area"
    np.testing.assert_almost_equal(ell.mean_radius(kind).value, 6371007.1810, decimal=4)
    kind = "same_volume"
    np.testing.assert_almost_equal(ell.mean_radius(kind).value, 6371000.7900, decimal=4)
...
def test_latitude_dependend_values():
    ...
    np.testing.assert_almost_equal(
        ell.meridian_curvature_radius(lat).value, 6359422.962, decimal=3
    )
    np.testing.assert_almost_equal(
        ell.prime_vertical_curvature_radius(lat).value, 6386175.289, decimal=3
    )
    np.testing.assert_almost_equal(
        ell.mean_curvature(lat).value, 1 / 6372785.088, decimal=16
    )
    ...
    np.testing.assert_almost_equal(
        ell.meridian_arc_distance(0.0 * u.deg, lat).value, 4186320.340, decimal=3
    )
...
def test_fwd():
    ...
    np.testing.assert_array_almost_equal(b_lat2.to("degree").value, lat2, decimal=8)
    np.testing.assert_array_almost_equal(b_lon2.to("degree").value, lon2, decimal=8)
    np.testing.assert_array_almost_equal(
        b_azi2.to("degree").value, azi2 - 180.0, decimal=8
    )
...
def test_inv():
    ...
    # check meridian arc
    np.testing.assert_almost_equal(
        ell.meridian_arc_distance(0.0 * u.deg, 90 * u.deg).value,
        ell.quadrant_distance.value,
        decimal=4,
    )
    ...
    np.testing.assert_almost_equal(
        ell.parallel_arc_distance(0.0 * u.deg, 0.0 * u.deg, 90.0 * u.deg).value,
        circle_length.value / 4,
        decimal=5,
    )
    ...
    np.testing.assert_almost_equal(
        ell.parallel_arc_distance(0.0 * u.deg, 0.0 * u.deg, 90.0 * u.deg).value,
        ell.inv(0.0 * u.deg, 0.0 * u.deg, 0.0 * u.deg, 90.0 * u.deg)[-1].value,
        decimal=5,
    )
    ...
    np.testing.assert_array_almost_equal(b_azi1.to("degree").value, azi1, decimal=3)
    np.testing.assert_array_almost_equal(
        b_azi2.to("degree").value, azi2 - 180.0, decimal=3
    )
    np.testing.assert_array_almost_equal(b_dist12.to("m").value, dist12, decimal=8)
...
def test_radiuses():
    ...
    np.testing.assert_array_almost_equal(
        ell.circle_radius(lat).value, np.sqrt(x**2 + y**2).value, decimal=5
    )

    sph_lat, _, sph_radius = geodetic_to_spherical(
        lat, lon=0.0 * u.deg, height=0.0 * u.m, ell=ell
    )

    np.testing.assert_array_almost_equal(
        ell.polar_equation(sph_lat).value, sph_radius.value, decimal=5
    )
...
def test_latitudes():
    ...
    np.testing.assert_array_almost_equal(
        ell.geocentric_latitude(lat).to("degree").value,
        sph_lat.to("degree").value,
        decimal=8,
    )

    reduced_latitude = np.degrees(np.arctan(np.tan(np.radians(sph_lat)) / (1 - ell.f)))

    np.testing.assert_array_almost_equal(
        ell.reduced_latitude(lat).to("degree").value,
        reduced_latitude.to("degree").value,
        decimal=8,
    )

    # compare with series from
    # J. P. Snyder, Map projections - a working  manual, 1926, page 16
    s2lat = (ell.e2 / 3 + 31 * ell.e2**2 / 180 + 59 * ell.e2**3 / 560) * np.sin(2 * lat)
