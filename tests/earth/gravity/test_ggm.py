...
def read_test_model():
    ...
    model = GlobalGravityFieldModel(
        cnm * u.dimensionless_unscaled, gm=gm * u.m**3 / u.s**2, r0=r0 * u.m, ell=ell
    )
    return model
...
def test_gravity():
    ...
    np.testing.assert_almost_equal(gravity_model.value, gravity_test)
...
def test_dov():
    ...
    dov_ew_model = model.vertical_deflection_ew(position).to("arcsec").value
    dov_ns_model = model.vertical_deflection_ns(position).to("arcsec").value
    dov_abs_model = model.vertical_deflection_abs(position).to("arcsec").value
...
def test_r_derivatives():
    ...
    r_derivative_differential_model = model._gravity.differential(position).d_distance

    np.testing.assert_almost_equal(
        r_derivative_model.value, r_derivative_differential_model.value
    )
...
def test_lat_derivatives():
    ...
    np.testing.assert_almost_equal(
        lat_derivative_model.value, lat_derivative_differential_model.value
    )
...
def test_lon_derivatives():
    ...
    np.testing.assert_almost_equal(
        lon_derivative_model.value, lon_derivative_differential_model.value
    )
